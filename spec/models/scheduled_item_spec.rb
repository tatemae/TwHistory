...
describe ScheduledItem do
  ...
  describe "named scopes" do
    ...
    describe "by_send" do
      before do
        ...
        @old_item = Factory(:scheduled_item, :send_at => 1.month.ago)
        @new_item = Factory(:scheduled_item, :send_at => 1.day.ago)
        @future_item = Factory(:scheduled_item, :send_at => 1.day.from_now)
      end
    ...
    describe "ready_to_send" do
      before do
        ScheduledItem.delete_all
        @ready_item = Factory(:scheduled_item, :send_at => 1.day.ago)
        @ready_item_too = Factory(:scheduled_item, :send_at => 1.minute.ago)
        @not_ready_item = Factory(:scheduled_item, :send_at => 1.day.from_now)
      end
  ...
  describe "broadcast_scheduled_items" do
    before(:each) do
      ...
      @ready_item = Factory(:scheduled_item, :broadcast => @broadcast, :send_at => 1.day.ago)
      @ready_item_too = Factory(:scheduled_item, :broadcast => @broadcast, :send_at => 1.minute.ago)
      @not_ready_item = Factory(:scheduled_item, :broadcast => @broadcast, :send_at => 1.day.from_now)
    end
